...
let rec run (p : prog) (st : stack) (e : env) (log : string list):
  ...
  | TryCatch (cmds1, cmds2) :: rest ->
    (match run cmds1 st e log with
     | (l, Ok x) -> run rest x e l
     | (l, error) -> (match run cmds2 (I_val (to_int_result error)::st) e l with
         |(lg, Ok x) -> run rest x e lg
         |(lg, err) -> lg, err))
  | Fun (fname, (arg, cmds)):: rest ->
    run rest st (put e fname (F_val (fname, arg, cmds, e))) log
  | Call::rest -> 
    (match st with 
     |x :: F_val (fname, arg, cmds, en)::st -> (match run cmds [] (put (put en fname (F_val (fname, arg, cmds, en))) arg x) log with 
         | (log, Ok (v :: _)) -> run rest (v :: st) e log
         | (log, Ok _) -> log, StackError
         | error -> error)
     |_ :: _ :: st -> log, TypeError
